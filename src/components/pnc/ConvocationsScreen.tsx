          ...
          <div key={conv.id} className={`${cardBg} rounded-xl p-4 shadow-sm transition-colors`}>
            <div className="flex items-start gap-3">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                conv.status === 'active' ? 'bg-[#1E5EFF]/10' : (darkMode ? 'bg-gray-700' : 'bg-gray-100')
              }`}>
                <FileText className={`w-5 h-5 ${conv.status === 'active' ? 'text-[#1E5EFF]' : textMuted}`} />
              </div>
              <div className="flex-1 min-w-0">
                <h4 className={`text-sm font-medium ${textPrimary}`}>{conv.title}</h4>
              ...
              <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                conv.status === 'active' ? 'bg-green-100 text-green-700' : (darkMode ? 'bg-gray-700 text-gray-400' : 'bg-gray-100 text-gray-500')
              }`}>
