...
const communes: Record<string, string[]> = {
  Kinshasa: ['Gombe', 'Barumbu', 'Bandundu', 'Bandalungwa', 'Kintambo', 'Kinshasa', 'Lingwala', 'Makala', 'Maluku', 'Masina', 'Matete', 'Mont-Ngafula', 'Ndjili', 'Ngaba', 'Ngaliema', 'Nsele', 'Selembao'],
}
...
export default function RegisterScreen() {
  ...
  const [form, setForm] = useState({
    name: '', phone: '', email: '', province: 'Kinshasa',
    commune: 'Gombe', password: '', confirmPassword: '',
    acceptTerms: false,
  })
    ...
    <div className="min-h-screen bg-white flex flex-col">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ex: Jean Mukendi" className="w-full px-4 py-3 bg-[#F5F6FA] rounded-xl text-sm border border-transparent focus:border-[#1E5EFF] focus:bg-white outline-none transition-all"
              />
            </div>
            <div>
              ...
              <input
                ...
                placeholder="+243 812 000 000" className="w-full px-4 py-3 bg-[#F5F6FA] rounded-xl text-sm border border-transparent focus:border-[#1E5EFF] focus:bg-white outline-none transition-all"
              />
            </div>
            <div>
              ...
              <input
                ...
                placeholder="email@exemple.com" className="w-full px-4 py-3 bg-[#F5F6FA] rounded-xl text-sm border border-transparent focus:border-[#1E5EFF] focus:bg-white outline-none transition-all"
              />
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                value={form.province} onChange={(e) => updateForm('province', e.target.value)}
        ...
        {step === 3 && (
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="Minimum 8 caractères" className="w-full px-4 py-3 bg-[#F5F6FA] rounded-xl text-sm border border-transparent focus:border-[#1E5EFF] focus:bg-white outline-none transition-all pr-12"
                />
            ...
            <div>
              ...
              <input
                type="password" value={form.confirmPassword} onChange={(e) => updateForm('confirmPassword', e.target.value)}
                placeholder="Confirmez votre mot de passe" className="w-full px-4 py-3 bg-[#F5F6FA] rounded-xl text-sm border border-transparent focus:border-[#1E5EFF] focus:bg-white outline-none transition-all"
              />
            </div>
            <div className="flex items-start gap-3 mt-2">
              <button
                onClick={() => updateForm('acceptTerms', !form.acceptTerms)}
              ...
              <p className="text-xs text-gray-500 leading-relaxed">
                ...
                <span className="text-[#1E5EFF] font-medium">politique de confidentialité</span> de PNC Alerte
              </p>
            </div>
          </div>
        )}
      ...
      <div className="px-6 pb-8 pt-4">
        ...
        <p className="text-center mt-4 text-sm text-gray-400">
          Déjà inscrit ?{' '}
          <button onClick={() => navigate('login')} className="text-[#1E5EFF] font-semibold">
            Se connecter
          </button>
        </p>
      </div>
